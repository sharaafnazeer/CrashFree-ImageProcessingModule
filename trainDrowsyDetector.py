...
def resolveFiles(subjectFolders):
    ...
    for subjectFolder in subjectFolders:
        ...
        for contentFolder in contentFolders:
            filesTemp = next(os.walk('Image/' + subjectFolder + '/' + contentFolder))[2]
            for alert in filesTemp:
                ...
                labels.append(contentFolder)
                data.append(image)

    return data, labels


data, labels = resolveFiles(next(os.walk('Image'))[1])
...
print("[INFO] generation image data...")
...
model.save("models/drowsyDetector.model", save_format="h5")
